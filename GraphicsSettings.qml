                        ...
                        ColumnLayout {
                            ...
                            RowLayout { Label { text: "图形增强" } Button { text: "?"; ToolTip.visible: hovered; ToolTip.text: "用AI提高帧数（适合看风景，但会加剧远处模糊感）。" }
                                ComboBox { model: ["未知","关", "开"];
                                    currentIndex: graphicsConfig.getImageSharpeningStatus(); onActivated: function(index) { graphicsConfig.setImageSharpening(index) } } }
                            ...
                            RowLayout { Label { text: "OpenGL GDI兼容性" } Button { text: "?"; ToolTip.visible: hovered; ToolTip.text: "对老旧游戏的优化。" }
                                ComboBox { model: ["未知","关","开","自动"];
                                    currentIndex: graphicsConfig.getOpenGLGDICompatibility(); onActivated: function(index) { graphicsConfig.setOpenGLGDICompatibility(index) } } }
                            ...
                            RowLayout { Label { text: "Vulkan/OpenGL 现行方法" } Button { text: "?"; ToolTip.visible: hovered; ToolTip.text: "针对第9代及以后CPU使用DX12的优化。" }
                                ComboBox { model: ["未知","关","开","自动"];
                                    currentIndex: graphicsConfig.getOpenGLPresentMethod(); onActivated: function(index) { graphicsConfig.setOpenGLPresentMethod(index) } } }
                            RowLayout { Label { text: "三重缓冲" } Button { text: "?"; ToolTip.visible: hovered; ToolTip.text: "减少画面撕裂（适合看风景，但增加输入延迟）。" }
                                ComboBox { model: ["未知","关","开"];
                                    currentIndex: graphicsConfig.getTripleBuffer(); onActivated: function(index) { graphicsConfig.setTripleBuffer(index) } } }
                            ...
                            RowLayout { Label { text: "各向异性过滤" } Button { text: "?"; ToolTip.visible: hovered; ToolTip.text: "提升远处物体清晰度（显卡性能越高可设置越高）。" } 
                                ComboBox { 
                                    model: Array.from({length: 17}, (_, i) => i);
                                    currentIndex: graphicsConfig.getAnisotropicFiltering(); 
                            ...
                            RowLayout {
                                ...
                                ComboBox {
                                    model: Array.from({length: 241}, (_, i) => i)
                                    currentIndex: graphicsConfig.appIdleFPSLimit
                                    onActivated: function(index) { graphicsConfig.setAppIdleFPSLimit(index) }
                                }
                            }
                            RowLayout { Label { text: "垂直同步" } Button { text: "?"; ToolTip.visible: hovered; ToolTip.text: "同步GPU渲染速率与显示器刷新率（适合看风景）。" }
                                ComboBox { model: ["未知","使用3D应用程序设置","关", "开","VSYNCMODE_FLIPINTERVAL2","VSYNCMODE_FLIPINTERVAL3","VSYNCMODE_FLIPINTERVAL4","VSYNCMODE_VIRTUAL"];
                                    currentIndex: graphicsConfig.getVSyncMode(); onActivated: function(index) { graphicsConfig.setVSyncMode(index) } } }
                            RowLayout { Label { text: "多重采样" } Button { text: "?"; ToolTip.visible: hovered; ToolTip.text: "提升画面细腻度（3060以下显卡建议关闭）。" } 
                                ComboBox { 
                                    model: ["关", "2x", "4x", "8x", "16x"]
                                    currentIndex: graphicsConfig.getAAModeMethod()
                                    onActivated: graphicsConfig.setAAModeMethod(index)
                                } 
                            }
                            RowLayout { Label { text: "平滑处理 - FXAA" } Button { text: "?"; ToolTip.visible: hovered; ToolTip.text: "消除物体边缘锯齿。" } 
                                ComboBox { 
                                    model: ["未知","关","开"]
                                    currentIndex: graphicsConfig.getFXAAEnable() ? 2 : 1
                                    onActivated: graphicsConfig.setFXAAEnable(index === 2)
                                } 
                            }
                            ...
                            RowLayout { Label { text: "平滑处理 - 灰度纠正" } Button { text: "?"; ToolTip.visible: hovered; ToolTip.text: "优化画面对比度。" } 
                                ComboBox { 
                                    model: ["应用程序控制", "开", "关"]
                                    currentIndex: graphicsConfig.getAAGammaCorrection()
                                    onActivated: graphicsConfig.setAAGammaCorrection(index)
                                } 
                            }
                            RowLayout { Label { text: "平滑处理 - 设置" } Button { text: "?"; ToolTip.visible: hovered; ToolTip.text: "托管对象的抗锯齿处理。" } ComboBox { model: [] } }
                            RowLayout { Label { text: "平滑处理 - 透明度" } Button { text: "?"; ToolTip.visible: hovered; ToolTip.text: "提升细节物体清晰度（显卡性能越高可设置越高，影响帧数）。" } ComboBox { model: ["关", "2x", "4x", "8x"]
                                currentIndex: graphicsConfig.getAATransparency()
                                onActivated: graphicsConfig.setAATransparency(index)
                                } }
                            ...
                            RowLayout { Label { text: "电源管理模式" } Button { text: "?"; ToolTip.visible: hovered; ToolTip.text: "控制显卡功耗策略（游戏推荐最大性能）。" } 
                                ComboBox { model: ["未知","自适应","最大性能","驱动控制","一致性能","最小功耗","最优电源"];
                                    currentIndex: graphicsConfig.getPowerManagementMode(); onActivated: function(index) { graphicsConfig.setPowerManagementMode(index) } }}
